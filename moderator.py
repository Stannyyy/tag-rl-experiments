...
class Moderator:
    ...
    def play_many(self, learn=True, competition_game=False):
        ...
        if competition_game:
            number_of_games = self._config.number_of_competition_episodes
        else:
            number_of_games = self._config.number_of_episodes_per_round

        # Initialize games
        self._games = [Game(self._config, self._display_games_path) for episode in range(number_of_games)]
        [self._games[episode].init_random_game() for episode in range(number_of_games)]
        self._turn_counts = [0 for episode in range(number_of_games)]
        game_overs = [False for episode in range(number_of_games)]
        ...
        unique_players = list(set(self._players))
        for player in unique_players:
            player.memory_many = [Memory(self._config) for episode in range(number_of_games)]
        selection = list(range(number_of_games))

        for i in range(self._config.maximum_steps+self._config.number_of_players):

            # Print progress
            print(f"\rPlaying many games, step {i} out of {self._config.maximum_steps}; {len(selection)} games are unfinished", end='')
            ...
            selection = [episode for episode in range(number_of_games) if not game_overs[episode]]

            # Make a move! Unless a tag has happened or the game has reached maximum turns
            choices = player.choose_many_actions(player.current_options, selection, competition_game=competition_game)
            choices = [8 if self._games[episode].ended > 0 else choices[episode] for episode in range(number_of_games)]

            # Set new experience
            rewards = [self._games[episode].move(self._turn, choices[episode]) for episode in range(number_of_games)]
            ...
            player.total_reward_tagger /= number_of_games
            player.total_reward_runner /= number_of_games
            ...
            # Next turn
            self.next_turn()
            self._turn_counts = [self._turn_counts[episode] if self._games[episode].ended else self._turn_counts[episode] + 1 for episode in range(number_of_games)]
            ...
            for episode in range(number_of_games):
                ...
                for episode_experience in episode_experiences:
                    ...
                    if (episode_experience not in unique_episode_experiences) & (count_end_states <= self._config.number_of_players):
                        ...
                        player.add_episode_experience(episode_experience)
            ...
            if learn & player.is_agent:
                if player.config.redo_batch:
                    ...
                    for i in range(num_batches):
                        print(f"\rRedoing batch, {i+1} out of {num_batches}", end='')
                        player.learn_by_replay(player.config.batch_size, epochs=10, verbose=False)
                else:
